function [metric, routing, batteries] = dsdvUpdaterBatteryRouting(routing, connMatrix, batteries, routeUse)
    ...
    while(metric.numRoute ~= oldNumData)
        ...
        for n = 1:length(connMatrix)
            ...
            for x = 1:length(neighbors)
                ...
                if routing(n, neighbor, 2) > (cost(n) + cost(neighbor))
                    routing(n, neighbor, 1) = neighbor;
                    routing(n, neighbor, 2) = cost(n) + cost(neighbor);
